...
class CampaignsPage():
    ...
    def __init__(self, camp_no):
        ...
        res = requests.get(f"http://fastapi-app/campaign/{self.camp_no}").json()
        self.df = pd.DataFrame(res).T
    ...
    def plot_campaign_sales(self):
        ...
        first = self.campaign_summary['First Day'][camp_no]
        last = self.campaign_summary['Last Day'][camp_no]
        total_days = self.campaign_summary['Duration'][camp_no]
        ...
        val = self.campaign_summary['Listed Products Sales During'][camp_no] / (last - first) + 1
        ax.plot((first, last), (val, val) , color='red', lw=3, ls='--', label=f'Avg. during {round(val,2)}')

        divisor = (trans_max - last) + 1
        if divisor < 1:
            divisor = 1
        val = self.campaign_summary['Listed Products Sales After'][camp_no] / divisor
        ax.plot((last, trans_max), (val, val) , color='blue', lw=3, ls='--', label=f'Avg. after {round(val,2)}')

        val = self.campaign_summary['Listed Products Sales Before'][camp_no] / (first - trans_min) + 1 
        ax.plot((trans_min, first), (val, val) , color='purple', lw=3, ls='--', label=f'Avg. before {round(val,2)}')

        val = self.campaign_summary['Listed Products Total Sales'][camp_no] / ((trans_max - trans_min) +1)
